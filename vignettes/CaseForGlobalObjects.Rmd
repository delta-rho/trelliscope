...
makeDisplay(byCounty,
   ...
   desc    = "List price per square foot vs. time") %>% suppressMessages
```
...
```{r, message=FALSE}
library(data.table)
housing100 = lapply(1:3, function(i){
		x = housing
...
makeDisplay(div, name ="tmp", panelFn=panelFn, detect.globals=FALSE) %>% suppressMessages
rm("meta")
```
```{r, eval=FALSE}
view(conn=conn)  
