...
webSync <- function(
   ...
   if(fixPermissions) {
      ...
      lns <- capture.output(system(paste(sshString, sshQuote, "chown -R shiny ", webConn$appDir, "/", webConn$name, sshQuote, sep = ""), intern = TRUE, ignore.stderr = FALSE, ignore.stdout = FALSE))
      if(verbose)
         cat(paste(c("*** Output ***", lns), collapse = "\n"))
   }
